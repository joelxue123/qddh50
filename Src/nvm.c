...
// STM32G431 has 128KB flash organized as 64 pages of 2KB each
// Using last two pages (63 and 62) for configuration
// Refer to RM0440 Rev 7 Section 3.3.1

#define FLASH_PAGE_SIZE          0x800U    // 2KB per page
#define FLASH_PAGE_62           62U        // Second to last page
#define FLASH_PAGE_63           63U        // Last page
#define FLASH_BASE_ADDR         0x08000000U

#define FLASH_SECTOR_10_BASE    (const volatile uint8_t*)(FLASH_BASE_ADDR + (FLASH_PAGE_62 * FLASH_PAGE_SIZE))
#define FLASH_SECTOR_10_SIZE    FLASH_PAGE_SIZE
#define FLASH_SECTOR_11_BASE    (const volatile uint8_t*)(FLASH_BASE_ADDR + (FLASH_PAGE_63 * FLASH_PAGE_SIZE))  
#define FLASH_SECTOR_11_SIZE    FLASH_PAGE_SIZE

// Update error flags for STM32G4
#define HAL_FLASH_ClearError() __HAL_FLASH_CLEAR_FLAG(FLASH_FLAG_ALL_ERRORS)
...
// Update erase function for STM32G4
int erase(sector_t *sector) {
    FLASH_EraseInitTypeDef erase_struct = {
        .TypeErase = FLASH_TYPEERASE_PAGES,    // Changed from SECTORS to PAGES
        .Page = sector->sector_id,             // Using page number instead of sector
        .NbPages = 1,                          // Erase one page
        .Banks = FLASH_BANK_1                  // Always Bank 1 for G431
    };
...
int set_allocation_state(sector_t *sector, size_t index, size_t count, field_state_t state) {
    ...
    for (; count >= 4; count -= 4, index += 4) {
        dword_data = ((uint64_t)(states | mask) << 56) | 
                    ((uint64_t)(states | mask) << 48) |
                    ((uint64_t)(states | mask) << 40) |
                    ((uint64_t)(states | mask) << 32) |
                    ((uint64_t)(states | mask) << 24) |
                    ((uint64_t)(states | mask) << 16) |
                    ((uint64_t)(states | mask) << 8) |
                    (uint64_t)(states | mask);
        
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, 
                (uintptr_t)&sector->alloc_table[index >> 2], 
                dword_data) != HAL_OK)
            goto fail;
        mask = 0;
    }
...
int NVM_write(size_t offset, uint8_t *data, size_t length) {
    ...
    // handle unaligned start
    // Handle unaligned start with doubleword writes
    for (; (offset & 0x7) && length; ++data, ++offset, --length)
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD,
                ((uintptr_t)&target->data[target->index]) + offset, 
                *(uint64_t*)data) != HAL_OK)
            goto fail;

    // Write 32-bit values in fast mode
    for (; length >= 8; data += 8, offset += 8, length -= 8) {
        if (length == 8) {
            // Last word uses FAST_AND_LAST
            if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_FAST_AND_LAST,
                    ((uintptr_t)&target->data[target->index]) + offset, 
                    *(uint64_t*)data) != HAL_OK)
                goto fail;
        } else {
            // Regular fast programming
    ...
    // Handle remaining bytes with doubleword writes
